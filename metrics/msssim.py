...
class _MS_SSIM(torch.nn.Module):
    def __init__(
        self,
        data_range: float = 255,
        size_average: bool = True,
...
class MSSSIM(SimpleMetric):
    """Multi-Scale Structural Similarity (MS-SSIM). Ranges from 1 (best) to 0 (worst).
    """

    def __init__(self, input_range: Tuple[int, int]=(-1, 1), device: Optional[str]=None) -> None:
        super().__init__(input_range, device)

        self.crit = _MS_SSIM(data_range=1.0).to(self.device)
